...
| BVN verification | `POST /api/kyc/bvn` | `/identity/bvn` | POST | `bvn` plus local Tier 1 support fields | Bearer secret | Provider identity details/status | n/a | Current | Endpoint current; success now triggers Maplerad Tier 1 upgrade and raw BVN is redacted in stored metadata |
| Create NGN virtual account | `POST /api/wallet/create/{userId}` | `/collections/virtual-account` | POST | `customer_id`, `currency`, optional `preferred_bank` | Bearer secret | `account_number`, `bank_name`/`bank.name`, `id` | `collection.successful` for credits | Previous code used obsolete `/issuing/virtual_accounts` | Patched endpoint and response mapping; preflights provider customer tier and maps Tier 1 failures to `CUSTOMER_NOT_TIER1` |
